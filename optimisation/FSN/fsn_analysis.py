...
class FSNanalysis(optimisationResult):
    
    def __init__(self,dir_path):

        self.synaptic_amp_control_dopamine = list()
        self.synaptic_amp_result_dopamine = list()
        self.synaptic_amp_passing_analysis_traces_dopamine = list()
        self.synaptic_amp_passing_traces_dopamine = list()
        self.synaptic_amp_all_traces_dopamine = list()
        
        super(FSNanalysis,self).__init__(dir_path=dir_path)

    def load_current(self):

        self.currents = np.loadtxt(self.dir_path / 'current.csv')
        ...
        self.current_modulation_pass = np.loadtxt(self.dir_path /  'current_modulation_pass.csv')

        self.dt = self.currents[0][1]-self.currents[0][0]
    ...
    def dopamine_analysis(self):
        ...
        self.synaptic_amp_control_dopamine.append(sf.synaptic_amplitude(self.voltages[-1],criteria['parameters']))


        for volt in self.voltage_modulation_pass:

            self.synaptic_amp_passing_traces_dopamine.append(sf.synaptic_amplitude(volt,criteria['parameters']))

        for volt in self.voltages[1:-1]:

            self.synaptic_amp_all_traces_dopamine.append(sf.synaptic_amplitude(volt,criteria['parameters']))

        synaptic_amp_results_dopamine = list()
        
        for volt in self.voltages[1:-1]:
            
            result = sc.membrane_amplitude_increase(criteria,[self.voltages[-1],volt])

            self.synaptic_amp_result_dopamine.append(result)

            synaptic_amp_results_dopamine.append([result['zscore'], result['diff']])

        index = np.where((np.transpose(synaptic_amp_results_dopamine)[0] < criteria['selection']['threshold']) & (np.transpose(synaptic_amp_results_dopamine)[1] > 0))
            
        synaptic_amp_passing_analysis = np.take(np.array(self.voltages[1:-1]),index,axis=0)[0]

        for volt_pass in synaptic_amp_passing_analysis:

            self.synaptic_amp_passing_analysis_traces_dopamine.append(sf.synaptic_amplitude(volt_pass,criteria['parameters']))

    def get_dopamine_control(self):

        return self.synaptic_amp_control_dopamine
        
    def save_selected_modulation(self):

        criteria = self.modulation_setup['selection_criteria'][0]['criteria']
        synaptic_amp_result = list()
        
        for volt in self.voltages[1:-1]:
            
            result = sc.membrane_amplitude_increase(criteria,[self.voltages[-1],volt])

            synaptic_amp_result.append([result['zscore'], result['diff']])

        index_original = np.where((np.transpose(synaptic_amp_result)[0] < criteria['selection']['threshold']) & (np.transpose(synaptic_amp_result)[1] > 0))

        results_pass = np.take(synaptic_amp_result,index_original,axis=0)[0]
