...
def plot_dopamine_acetylcholine_control(obj_loaded_traces,type_cell='dSPN'):
    ...
    time = np.linspace(0,len(control[0])*0.5,len(control[0]))
    for i in np.arange(1,len(ach),1):
            fig = plt.figure()           
            ax = fig.add_subplot(111)
            ax.plot(time,control[i]*1e3,c='black')
            ax.plot(time,ach[i]*1e3,c='purple')
            ax.set_ylabel('Membrane potential (mV)')
            ax.set_xlabel('Time(ms)')
            ...
            fig.savefig('dopamine-acetylcholine-trial-network/analysis/dopamine-acetylcholine-control-traces'+str(i)+type_cell+".svg")  
